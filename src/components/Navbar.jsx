...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      {/* ISO and ICV Icons - Top Right */}
      <div className="hidden md:flex items-center gap-4 absolute top-2 right-4 z-50">
        <HashLink to="#iso-certified" smooth>
          <motion.img
            src={iso}
            alt="ISO Certification"
            className="w-20 h-20 object-contain rounded-sm shadow-sm cursor-pointer"
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center text-base font-semibold px-3 py-2 rounded-md transition-all duration-200 ${
                      isActive
                        ? "text-blue-500 bg-blue-50/30 backdrop-blur-sm"
                        : "text-blue-600 hover:text-blue-400 hover:bg-blue-50/20 hover:backdrop-blur-sm"
                    }`
                  }
                  onClick={() => setMobileOpen(false)}
                >
